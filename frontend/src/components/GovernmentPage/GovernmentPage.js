...
function GovernmentPage() {
  ...
  // fetch list of sellers (for suspension) using your pattern: /seller (singular)
  const fetchSellers = async () => {
    try {
      ...
      alert("Failed to load sellers. Endpoint /seller returned error or is missing.");
      setSellers([]);
    }
  };

  // fetch list of farmers (for suspension) using your farmer router: /farmer
  const fetchFarmers = async () => {
    try {
      ...
      alert("Failed to load farmers. Endpoint /farmer returned error or is missing.");
      setFarmers([]);
    }
  };
  ...
  const suspendFarmer = async (farmer) => {
    ...
    try {
      // use your farmer delete path exactly: /farmer/delete/:id
  ...
  const suspendDeliveryMan = async (dm) => {
    ...
    try {
      // keep using /deliverymen/:id delete (unchanged)
      await axios.delete(`${BASE_URL}/deliverymen/${dm._id}`);
  ...
  const suspendSeller = async (seller) => {
    ...
    try {
      // use seller delete path similar to farmer: /seller/delete/:id
