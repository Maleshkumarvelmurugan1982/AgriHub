...
function GovernmentPage() {
  ...
  // Sellers & Farmers management (fetched from respective routes)
  ...
  // Keep track of which endpoint worked for each resource so we delete with the same base
  const [farmersEndpoint, setFarmersEndpoint] = useState(null);
  const [sellersEndpoint, setSellersEndpoint] = useState(null);
  const [deliverymenEndpoint, setDeliverymenEndpoint] = useState("/deliverymen"); // default used elsewhere
  ...
  // Helper: try multiple endpoints in order until one succeeds.
  // Returns { data, endpoint } where endpoint is the path that worked (including leading slash).
  const tryGetFromEndpoints = async (paths) => {
    let lastError = null;
    for (const p of paths) {
      const path = p.startsWith("/") ? p : `/${p}`;
      try {
        const res = await axios.get(`${BASE_URL}${path}`);
        // Accept any 2xx response with data (array or object)
        return { data: res.data, endpoint: path };
      } catch (err) {
        lastError = err;
        // continue to next candidate on any error (404, network, etc.)
      }
    }
    // after trying all, rethrow last error so caller can handle
    throw lastError || new Error("No endpoints provided");
  };
  ...
  // Export only currently-visible delivery history to CSV
  ...
  // Copy visible history CSV to clipboard
  ...
  // Keep previous full CSV export (unchanged)
  ...
  const fetchDeliveryMen = async () => {
    // try a few candidate endpoints if possible
    const candidates = ["/deliverymen", "/deliveryman", "/users?role=deliveryman", "/users?role=deliverymen"];
    try {
      const { data, endpoint } = await tryGetFromEndpoints(candidates);
      setDeliveryMen(Array.isArray(data) ? data : []);
      setDeliverymenEndpoint(endpoint.split("?")[0] || "/deliverymen");
    } catch (err) {
      console.error("Failed to fetch delivery men:", err);
      alert("Failed to load delivery men. The server returned an error or the endpoint is missing.");
      setDeliveryMen([]);
    }
  };

  // fetch list of sellers (for suspension) - tries multiple possible endpoints
  const fetchSellers = async () => {
    const candidates = ["/sellers", "/seller", "/users?role=seller", "/users?role=sellers"];
    try {
      const { data, endpoint } = await tryGetFromEndpoints(candidates);
      setSellers(Array.isArray(data) ? data : []);
      setSellersEndpoint(endpoint.split("?")[0] || "/sellers");
    } catch (err) {
      console.error("Failed to fetch sellers:", err);
      alert("Failed to load sellers. The server returned 404 or the endpoint path differs. Tried: /sellers, /seller, /users?role=seller");
      setSellers([]);
      setSellersEndpoint(null);
    }
  };

  // fetch list of farmers (for suspension) - tries multiple possible endpoints
  const fetchFarmers = async () => {
    const candidates = ["/farmers", "/farmer", "/users?role=farmer", "/users?role=farmers"];
    try {
      const { data, endpoint } = await tryGetFromEndpoints(candidates);
      setFarmers(Array.isArray(data) ? data : []);
      setFarmersEndpoint(endpoint.split("?")[0] || "/farmers");
    } catch (err) {
      console.error("Failed to fetch farmers:", err);
      alert("Failed to load farmers. The server returned 404 or the endpoint path differs. Tried: /farmers, /farmer, /users?role=farmer");
      setFarmers([]);
      setFarmersEndpoint(null);
    }
  };
  ...
  //
  // Suspension (hard delete) features using same fetch routes only.
  // No audit, no localStorage, no /suspensions endpoints used.
  //

  // Build delete URL from the successful endpoint base. endpointBase is like "/farmers" or "/users"
  const buildDeleteUrl = (endpointBase, id) => {
    const base = endpointBase ? endpointBase.split("?")[0] : null;
    if (!base) return null;
    return `${BASE_URL}${base.replace(/\/$/, "")}/${id}`;
  };
  ...
  const suspendFarmer = async (farmer) => {
    ...
    try {
      // use the same endpoint base that worked for fetching; fallback to /farmers
      const deleteUrl = buildDeleteUrl(farmersEndpoint || "/farmers", farmer._id) || `${BASE_URL}/farmers/${farmer._id}`;
      await axios.delete(deleteUrl);
      setFarmers((prev) => prev.filter((f) => f._id !== farmer._id));
      alert("Farmer suspended and deleted from database.");
    } catch (err) {
      console.error("Failed to suspend farmer:", err);
      alert("Failed to suspend (delete) farmer. Please try again.");
    }
  };
  ...
  const suspendDeliveryMan = async (dm) => {
    ...
    try {
      const deleteUrl = buildDeleteUrl(deliverymenEndpoint || "/deliverymen", dm._id) || `${BASE_URL}/deliverymen/${dm._id}`;
      await axios.delete(deleteUrl);
      setDeliveryMen((prev) => prev.filter((d) => d._id !== dm._id));
      alert("Deliveryman suspended and deleted from database.");
    } catch (err) {
  ...
  const suspendSeller = async (seller) => {
    ...
    try {
      const deleteUrl = buildDeleteUrl(sellersEndpoint || "/sellers", seller._id) || `${BASE_URL}/sellers/${seller._id}`;
      await axios.delete(deleteUrl);
      setSellers((prev) => prev.filter((s) => s._id !== seller._id));
      alert("Seller suspended and deleted from database.");
    } catch (err) {
      console.error("Failed to suspend seller:", err);
      alert("Failed to suspend (delete) seller. Please try again.");
    }
  };
