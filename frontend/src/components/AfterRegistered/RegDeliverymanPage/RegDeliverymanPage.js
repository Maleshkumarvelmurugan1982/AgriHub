...
function RegDeliverymanPage() {
  ...
  // New states for deliveries summary modal
  ...
  // Removed the previous client-side distance calculation as requested.
  // Distance should come from backend if available (order.distance) — otherwise we will not display it.

  // Get all unique districts from orders (districts expected from backend)
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Item Name", "Quantity", "Price (Rs.)", "Order Date", "Delivery Date", "From (Farmer)", "Farmer Contact", "To (Seller)", "Seller Contact", "District", "Status", "Current Salary (Rs.)"];
    ...
    history.forEach(order => {
      ...
      const district = order.district || 'N/A';
  ...
  // Theme persistence on mount
  useEffect(() => {
    const savedTheme = localStorage.getItem("delivery_dashboard_theme");
    if (savedTheme) {
      const isDark = savedTheme === "dark";
      setDarkMode(isDark);
      if (isDark) document.body.classList.add("delivery-dark");
      else document.body.classList.remove("delivery-dark");
    }
  }, []);

  // Keep body class and localStorage in sync with darkMode
  useEffect(() => {
    if (darkMode) {
      document.body.classList.add("delivery-dark");
      localStorage.setItem("delivery_dashboard_theme", "dark");
    } else {
      document.body.classList.remove("delivery-dark");
      localStorage.setItem("delivery_dashboard_theme", "light");
    }
  }, [darkMode]);
  ...
  const OrderCard = ({ order, type, isAvailable }) => {
    // We no longer calculate distance on the client side.
  ...
  // Builds deliveries summary from backend-fetched orders (delivered/approved)
  const buildDeliveriesSummary = () => {
    const history = getDeliveryHistory();
    const summaryList = history.map(order => {
      const hasFarmerInfo = order.farmerId && typeof order.farmerId === 'object';
      const farmerName = hasFarmerInfo ? `${order.farmerId.fname || ''} ${order.farmerId.lname || ''}`.trim() : 'Unknown';
      const farmerLocation = (order.farmerId && (order.farmerId.district || order.farmerId.location || order.farmerId.address || order.farmerId.city)) || order.farmerLocation || order.district || 'Unknown';

      const hasSellerInfo = order.sellerId && typeof order.sellerId === 'object';
      const sellerName = hasSellerInfo ? `${order.sellerId.fname || ''} ${order.sellerId.lname || ''}`.trim() : 'Unknown';
      const sellerLocation = (order.sellerId && (order.sellerId.district || order.sellerId.location || order.sellerId.address || order.sellerId.city)) || order.sellerLocation || order.district || 'Unknown';
    ...
    setDeliveriesSummary(summaryList);
  };

  // Trigger summary modal (ensure data is built from backend fetched orders)
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: darkMode ? '#1a202c' : '#f8f9fa', color: darkMode ? 'white' : 'black' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '16px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              ...
              {/* New button: View Deliveries Summary */}
              <button
              ...
              >
                ...
                <span className="hidden-mobile">View Deliveries Summary</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '14px', color: '#6c757d' }}>Total Deliveries</div>
                <div style={{ fontWeight: '700', fontSize: '20px' }}>{deliveriesSummary.length}</div>
              </div>
            </div>
              ...
              <div style={{ display: 'grid', gap: '12px' }}>
                {deliveriesSummary.map((d) => (
                  <div key={d.orderId} style={{
                    ...
                    <div>
                      <div style={{ fontWeight: '700', fontSize: '16px' }}>{d.item}</div>
                      <div style={{ fontSize: '13px', color: '#6c757d', marginTop: '6px' }}>
                        From: <strong>{d.fromName}</strong> — <span style={{ fontStyle: 'italic' }}>{d.fromLocation}</span>
                      </div>
                      <div style={{ fontSize: '13px', color: '#6c757d', marginTop: '4px' }}>
                        To: <strong>{d.toName}</strong> — <span style={{ fontStyle: 'italic' }}>{d.toLocation}</span>
                      </div>
                      <div style={{ fontSize: '12px', color: '#6c757d', marginTop: '6px' }}>
                        Delivered on: {formatDate(d.date)}
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px' }}>
                          ...
                          {order.district && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: '#fff3e0', padding: '8px', borderRadius: '6px' }}>
                              <FontAwesomeIcon icon={faMapMarkerAlt} style={{ color: '#f57c00' }} />
                              <span style={{ fontWeight: '500', fontSize: '14px' }}>{district}</span>
                            </div>
                          )}
                        </div>
      ...
      <style>{`
        ...
        /* Simple body class for persisted theme */
        .delivery-dark {
